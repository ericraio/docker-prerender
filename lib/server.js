...
server.createPhantom = function() {
    ...
    console.log('starting phantom on port [' + port + ']');
...
server.checkIfJavascriptTimedOut = function(req, res) {
    ...
    } else if (false) { //(timeout && lastJsExecutionWasLessThanTwoSecondsAgo) {
...
server.onPageEvaluate = function(req, res) {
    ...
    console.log('Prerender stage: ', req.prerender.stage);
    if(req.prerender.stage >= 2) return;
...
server._killPhantomJS = function() {
    ...
    require('tree-kill')(this.options.worker.process.pid, 'SIGTERM');
       ...
       //     console.log('pkilling phantomjs');
